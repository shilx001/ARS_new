...
class Policy:
    ...
    def update_by_ddpg(self, gradient):
        self.theta += gradient
...
class ARSTrainer:
    ...
    def explore(self, direction=None, delta=None, print_done=False, store_transition=False):
        ...
        while not done and num_plays < self.hp.episode_length:  # exectution
            ...
            origin_state = state  # un-normalized state
            ...
            if store_transition:
                self.ddpg_trainer.store_transition(state, action, reward, next_state, done)
            state = next_state
            sum_rewards += reward
            num_plays += 1
    ...
    def train(self):
        ...
        for step in range(self.hp.nb_steps):
            ...
            for k in range(self.hp.num_deltas):
                positive_rewards[k] = self.explore(direction="+", delta=deltas[k], store_transition=True)
                negative_rewards[k] = self.explore(direction="-", delta=deltas[k], store_transition=True)
